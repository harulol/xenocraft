...
import dev.hawu.plugins.xenocraft.data.ClassNation
...
object SkillsGUI extends ModuleHolder("skills-ui"):
  ...
  def openSkills(player: Player): Unit =
    ...
    user.masterSkills.zipWithIndex
      .foreach((skill, index) => model.mount(index + 32, new MasterSkillComponent(user, player, index)))
  ...
  private def getSkillDisplay(skill: SkillType, master: Boolean = false)(using player: Player): ItemStack =
    ...
    if skill == null then
      I18n.translateItem(
        ...
        if master then "empty-master-skill" else "empty-class-skill",
      )
    else
  ...
  /** The component for handling clicks on master skills
    */
  class MasterSkillComponent(private var user: User, private var player: Player, private val slot: Int)
