...
/**
 * The dedicated singleton object for managing GUI
 * for choosing a character preset.
 */
object CharactersGUI extends ModuleHolder("characters-ui"):
  ...
  def openCharacters(player: Player): Unit =
    ...
    GuiPaginator.newBuilder[Character]()
      ...
      .setBackSlots(Set.empty[Int].map(Integer.valueOf).asJava)
      .setPredicate(null)
      .setCollection(Character.values.toList.asJava)
      .setFilterSlots(Set.empty[Int].map(Integer.valueOf).asJava)
      .setItemGenerator((char, _) => new GuiComponent[Unit]() {
        private val item = I18n.translateItem(char.icon -> 1, "character",
