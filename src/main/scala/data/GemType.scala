...
enum GemType(
  ...
  val value1: (Double, Double),
  val value2: (Double, Double) = (0, 0),
  ...
  def name(locale: Locale, level: Int = 0): String =
    ...
    if level > 10 || level < 1 then localizedName else s"$localizedName ${I18n.toRomans(level)}"
  ...
  /** Retrieves the localized description for the gem.
    ...
    */
  def description(locale: Locale, level: Int = 10): String = GemsGUI.getModule.translate(
    locale,
    s"${this.toString.replace('_', '-').toLowerCase}-desc",
    Pair.of("value1", format(value1At(level), true)),
    Pair.of("value2", format(value2At(level))),
  )

  private def format(num: Double, checkPercentage: Boolean = false): String =
    if checkPercentage && this.percentage then s"${(num * 100).intValue.toString}%" else num.intValue.toString

  /** Calculates the increase of the stat if the gem is at the specified level, using the values from value1 tuple.
    *
    * @param level
    *   the level
    * @return
  ...
  /** Calculates the increase of the stat if the gem is at the specified level, using the values from value2 tuple.
    *
    * This only exists to serve a few Healer's gems, most other gems don't have this attribute.
    *
    * @param level
    *   the level
    * @return
