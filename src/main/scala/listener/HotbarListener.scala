...
object HotbarListener extends Listener:
  ...
  private def onSlotChange(event: PlayerItemHeldEvent): Unit =
    ...
    if user.bladeUnsheathed then
      ...
      event.getNewSlot match
        case 2 => trySingleArt(event.getPlayer, user.masterArts(0), true)
        case 3 => trySingleArt(event.getPlayer, user.masterArts(1), true)
        case 4 => trySingleArt(event.getPlayer, user.masterArts(2), true)
        case 5 => tryFusion(event.getPlayer, 0)
        case 6 => tryFusion(event.getPlayer, 1)
        case 7 => tryFusion(event.getPlayer, 2)
        case 8 => trySingleArt(event.getPlayer, user.talentArt.orNull, true)
        case _ => ()

  private def trySingleArt(player: Player, art: ArtType, master: Boolean): Unit =
    ...
    ArtManager.useSingleArt(player, art, master)
  ...
  private def tryFusion(player: Player, index: Int): Unit =
    ...
    if masterArt == null || user.isOnCooldown(masterArt) then
      ArtManager.useSingleArt(player, classArt, false)
      return ()

    // If both are off cooldown, and player is sneaking. It will be a fusion art.
    if player.isSneaking then ArtManager.useFusionArt(player, classArt, masterArt) else ArtManager.useSingleArt(player, classArt, false)
